...
class TiTokDiscriminator(nn.Module):
    def __init__(
        ...
        in_channel_depth = 2 * self.in_channels * self.temporal_patch_size * self.spatial_patch_size ** 2 # 2x for clip concat
    ...
    def forward(self, target, recon):
        orders = torch.rand(size=(target.shape[0], 1, 1, 1, 1)).to(target.device) < 0.5 # even?
